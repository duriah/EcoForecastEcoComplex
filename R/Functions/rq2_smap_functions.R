...
SMapInteractions <- function(Target, data, theta=NULL, num.clusters.target=1, 
  ...
  species_interactions <-mclapply(data.list, function(d){
    
    treat <- unique(d$treat)
    treat2 <- unique(d$treat2)
    treat3 <- unique(d$treat3)
    id <- unique(d$ID)
    
    Embedding <- createEmbedding(ccm_df, Target, id)
    ll <- housekeeping_smap(d, Target, Embedding)
    block<-ll$block
    Edim<-ll$Edim
    ...
    if(method=="SMap"){
      coeff <- ClassicSMap(Target, block, Embedding, Edim, theta, coeff_names, lib, pred)
      parameters <- list()
    }
    if(method=="RegSMap"){
...
createEmbedding <- function(ccm_df, Target, id){
  emb <- ccm_df %>%
    ungroup() %>%
    dplyr::filter(ID==id, target==Target, interactor!=Target) %>%
...
ClassicSMap <- function(Target, block, Embedding, Edim, theta, coeff_names,
                        lib, pred){
...
DetermineNonlinearity <- function(Target, data, thetas, Embedding=NULL, 
                                  ccm_df=NULL, createEmbedding=F, num.clusters=1){
  
  if(createEmbedding) Embedding <- createEmbedding(ccm_df, Target)
  
  theta_rho <- mclapply(thetas, function(theta){
    # estimate coefficients for a certain theta value
    coeff <- ClassicSMap(Target, data, Embedding, theta)
